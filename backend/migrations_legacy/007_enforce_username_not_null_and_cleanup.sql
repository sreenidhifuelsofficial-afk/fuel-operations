...
-- 0) Backfill any NULL emails before clearing sample emails (to avoid constraint violations)
UPDATE public.users
   SET email = COALESCE(username, full_name, id::text) || '@example.invalid'
 WHERE email IS NULL OR btrim(email) = '';

-- 1) Clear sample/demo emails (set to generated fallback instead of NULL)
UPDATE public.users
