...
// Re-exports the existing backend/db.js module so that all services
// share a single pool configuration and type-parser setup.
//
// Usage:
//   const pool = require('@fuel-ops/db');
...
// Parse NUMERIC as JS number (NUMERIC(14,3) for volume fields; float precision acceptable)
...
// Startup connectivity verification with retry/backoff (non-blocking)
pool.__ready = false;
...
async function verifyConnectivity() {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const backoff = Math.min(8000, Math.round(baseDelay * Math.pow(2, attempt - 1) * (0.75 + Math.random() * 0.5)));
